...
try{
    ...
    echo 'Create Error at UniqueId'.$e->getMessage();
}

try{
    $sqlscript1 = "SELECT * FROM `$tableName`";
    $getSqlId = $pdo->prepare($sqlscript1);
    $getSqlId->execute();
    $getEventIds = $getSqlId->fetchAll(PDO::FETCH_ASSOC);
}catch(PDOException $e){
    echo 'Event Id list: '.$e->getMessage();
}


// echo '<pre>';
// var_dump($getEventIds);
// echo '</pre>';

foreach($getEventIds as $getEventId){
    $matchingEvent = [];
    
    if($getEventId['eventId'] !== null){
        // echo $getEventId['eventId'].'test';
        try{
            $statment = $pdo->prepare('SELECT * FROM allevents WHERE id = :id');
            $statment->bindValue(':id', $getEventId['eventId']);
            $statment->execute();
            $matchingEvent = $statment->fetch(PDO::FETCH_ASSOC);
            // echo '<pre>';
            // var_dump($matchingEvent);
            // echo '</pre>';
            array_push($allEvents, $matchingEvent);
        }catch(PDOException $e){
            echo 'Errors at loop'.$e->getMessage();
        }
    }
                    ...
                    <?php  foreach($allEvents as $i => $event): ?>
                        <div class="card">
                            ...
                            <div class="btn-container">
                                <a href="update.php?id=<?php echo $event['id']; ?>" class="btn btn-primary">
                                    Edit
                                </a>
